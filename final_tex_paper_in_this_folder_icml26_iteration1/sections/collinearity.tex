...
\end{align}
Substituting \eqref{eq:JJ-embed} into \eqref{eq:collinearity-master} yields the sum over all batch slots $j$ where item $x_i$ appears:
\begin{align}
\Delta q_i \,=\, -\eta \sum_{j:\, x_j = x_i} g_j. \label{eq:update-sum-form}
\end{align}
Since each $g_j \perp q_i$ (by the cosine-loss orthogonality, Section~2.5), their sum is also orthogonal to $q_i$. Hence the update is strictly collinear with the aggregate gradient $\sum_{j: x_j = x_i} g_j$.
...
\end{align}
If the distinct inputs are pairwise orthogonal (e.g., one-hot features or, more generally, $x_i^{\!\top}x_j=0$ for $i\neq j$), the cross-terms vanish:
\begin{align}
\Delta q_i \,=\, -\eta\,\|x_i\|^2 \sum_{j:\,x_j=x_i} g_j,
\end{align}
which is again collinear with the aggregate gradient.
