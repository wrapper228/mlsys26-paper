...
\begin{itemize}
    ...
    (A3) per-item parameters are not shared (lookup-equivalent\footnote{\emph{Lookup-equivalent} means: embedding lookup (or one-hot $\to$ Linear) with no extra coordinate mixing or non-uniform scaling.}); and (A4) the loss depends on the item only via cosine. 
    Orthogonality breaks if any premise is violated.
    \item \textbf{Explanation of why orthogonality of embedding updates implies popularity bias.} We prove a formal mechanism (conditional on per-update increments) linking sampling frequency to norm growth and demonstrate that this geometric artifact translates directly into popularity biased retrieval outcomes.
