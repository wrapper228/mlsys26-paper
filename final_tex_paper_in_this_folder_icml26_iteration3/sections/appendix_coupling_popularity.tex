...
\begin{quote}
\textbf{Definition (Replay model).} Fix an item identity $i$. Consider any training run and let $N_T$ be the number of \emph{update events} for item $i$ up to time $T$. For each update event $k\in\{1,\dots,N_T\}$ define $\delta_k := \|\Delta q_i\|^2$, the squared displacement of the item embedding at that update. The \emph{replay model} treats the nonnegative sequence $\{\delta_k\}_{k\ge 1}$ as fixed (non-random) and compares only the effect of varying the sampling frequency (hence the update count $N_T$). We do \emph{not} claim that $\{\delta_k\}$ is invariant under changing the training sampling policy in full SGD dynamics.
\end{quote}
