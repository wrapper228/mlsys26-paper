...
If the following five factors hold simultaneously for the item tower, then item embeddings move orthogonally, their norms grow monotonically with updates, and a frequency--norm mechanism (conditional on per-update increments) for popularity bias is induced (Appendix~\ref{app:popularity-dependence}):
\begin{enumerate}
  ...
  \item each distinct input has a dedicated, non-shared parameter row (lookup-equivalent),
