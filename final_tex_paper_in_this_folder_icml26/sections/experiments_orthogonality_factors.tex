...
\textit{Clarifications.} In all experiments the right tower (item tower) is a simple embedding layer (nn.Embedding); InfoNCE \cite{oord2018cpc} denotes the contrastive softmax loss with in-batch negatives, where ``cos'' (resp., ``dot'') refers to cosine (resp., dot-product) similarity. I use SGD without momentum and without regularization; alternative optimizers did not reveal any movement patterns, although I will separately consider mixed-optimizer settings, including Adam, later (see Experiment~10). In Experiment~4 the architecture coincides with one-hot $\to$ Linear $\to$ Linear, i.e., simply two linear layers. The architecture in Experiment~5 is an example of an encoder that consists of a single Linear layer without the input-orthogonality requirement (no one-hot inputs): the inputs are just some numeric features.
...
Overall, these experiments confirm that strict orthogonality requires precise conditions: violations lead to non-systematic, chaotic movement. While I classify movement here as binary (orthogonal vs. chaotic), characterizing the continuous degree of degradation under partial violations remains an interesting direction for future work.