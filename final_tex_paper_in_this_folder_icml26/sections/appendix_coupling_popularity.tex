...
I aim to show that, after $T$ training batches, the embedding norm of item $i$ is nondecreasing in its sampling probability $p_i$:
\begin{equation}
\mathbb E\,\bigl\|q_i^{(T)}\bigr\| \;\text{ is nondecreasing in }\; p_i.
\end{equation}
I employ the coupling method from probability theory \cite{roch_mdp_2024}.

\paragraph{1) Two runs on the same randomness (I fixed the random outcome of batch generation).}
...
\paragraph{4) Induction over steps: why ``larger $\boldsymbol{p_i}$'' implies ``no smaller outcome''.}
...
\bigl\|q_i^{(T)}\bigr\| \text{ doesn't decrease on avg.} \\
\text{when } p_i \text{ is larger.}
\end{gathered}}
\end{equation}
