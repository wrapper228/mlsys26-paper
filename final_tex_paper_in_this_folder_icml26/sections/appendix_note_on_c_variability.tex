...
At a heuristic level, a sharp growth of the norm of $u$ would require that, simultaneously, all contributions $\hat k_i$ add up co-directionally in the tangent plane to $q$ — an event we consider unlikely under random batch formation. Even if for some reason this happens often, the sum of contributions is limited by the number of pairs with this item in the training set and by the value of $\tfrac{\partial F}{\partial cos}$, whose stability we rigorously justified above.
